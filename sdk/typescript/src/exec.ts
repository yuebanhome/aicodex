...
function findCodexPath(): CodexPathResolution {
  ...
  const nativePackage = resolveNativePackage(vendorRoot, targetTriple, aicodexBinaryName);
  if (!nativePackage) {
    throw new Error(
      `Unable to locate AICodex CLI binaries for ${targetTriple}. Ensure ${CODEX_NPM_NAME} is installed with optional dependencies.`,
    );
  }
...
export function resolveNativePackage(
  ...
  const legacyBinaryPath = path.join(packageRoot, "aicodex", codexBinaryName);
